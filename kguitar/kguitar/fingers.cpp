...
// GREYFIX!!!
static int tune[6]={40,45,50,55,59,64};
...
Fingering::Fingering(int strings,QWidget *parent,const char *name): QFrame(parent,name)
{
    ...
    setFixedSize(numstr*SCALE+2*BORDER+FRETTEXT+SCROLLER,NUMFRETS*SCALE+SCALE+2*BORDER+2*SPACER+NOTES);
...
void Fingering::drawContents(QPainter *p)
{
	...
	if (appl[i]==-1) {
	    ...
	    p->drawText(BORDER+FRETTEXT+i*SCALE,BORDER+NUMFRETS*SCALE+SCALE+2*SPACER,SCALE,30,
			AlignHCenter | AlignTop,note_name((appl[i]+tune[i])%12));
	}
    }

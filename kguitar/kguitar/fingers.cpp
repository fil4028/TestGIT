...
void Fingering::drawContents(QPainter *p)
{
    ...
    p->drawLine(BORDER+FRETTEXT,BORDER+SCALE+SPACER,BORDER+FRETTEXT+parm->string*SCALE,BORDER+SCALE+SPACER);
	...
	if (appl[i]==-1) {
		...
		p->drawEllipse(i*SCALE+BORDER+CIRCBORD+FRETTEXT,
			       BORDER+SCALE+2*SPACER+(appl[i]-ff->value())*SCALE+CIRCBORD,CIRCLE,CIRCLE);
	    };
	    p->drawText(BORDER+FRETTEXT+i*SCALE,BORDER+NUMFRETS*SCALE+SCALE+2*SPACER,SCALE,30,
			AlignHCenter | AlignTop,note_name((appl[i]+parm->tune[i])%12));
	}
    }
    ...
    for (int i=0;i<NUMFRETS;i++) {
	barre=0;
	while ((appl[parm->string-barre-1]>=(i+ff->value())) || (appl[parm->string-barre-1]==-1)) {
	    barre++;

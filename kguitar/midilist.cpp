...
void MidiData::getMidiList(TabTrack *trk, MidiList &ml, bool useTSE3 = FALSE)
{
	long timer = 0;
	...
	for (uint x = 0; x < trk->c.size(); x++) {
		...
		for (int i = 0; i < trk->string; i++) {
			...
			if (useTSE3) {
				ml.inSort(new MidiEvent(timer, duration, trk->channel - 1, MIDI_NOTEON, pitch, 0x60));
			} else {
				ml.inSort(new MidiEvent(timer, 0, trk->channel - 1, noteon, pitch, 0x60));
				ml.inSort(new MidiEvent(timer + duration, 0, trk->channel - 1, noteon, pitch, 0));
			}
		}
		timer += midilen;
	}
